...
### 1.1 当前遥感基础模型 (RSFM) 的局限性
- **单一模态依赖**  
  现有模型仅处理光学图像或 SAR 图像，未能融合多模态数据优势：
  - 光学图像：光谱带丰富但受天气影响
  - SAR 图像：全天候可用但缺乏纹理细节
- **时空建模缺失**  
  忽视时间序列对变化预测等任务的关键作用
- **地理上下文未被利用**  
  遥感数据的时空坐标包含区域/季节性语义，但未被有效建模
- **任务适应性不足**  
  无法支持多模态、多粒度（像素/对象/图像级）、多任务类型（分类/定位）的复杂需求
...
class FactorizedEncoder:
    def __init__(self):
        self.spatial_encoder = SpatialCNN()       # 空间特征提取
        self.temporal_fuser = TemporalTransformer()  # 时间融合模块

    def forward(self, x_HR, x_Ms, x_SAR):
        # 多模态输入：高分辨率光学/多光谱/SAR
        hr_feat = self.spatial_encoder(x_HR)
        ms_feat = self.spatial_encoder(x_Ms)
        sar_feat = self.spatial_encoder(x_SAR)
        
        # 多模态时间融合
        fused_feat = self.temporal_fuser([hr_feat, ms_feat, sar_feat])
        return fused_feat
```
...
损失函数组成：
- **像素级**：`L_pixel = -log(sim(f_i, f_j) / τ)`  
- **对象级**：`L_object = -log(sim(ROI_pooling(f), cluster_center) / τ)`  
- **图像级**：`L_image = -log(sim(global_avg_pool(f), global_label) / τ)`  
...
- 区域原型库包含：
  ...
  # 示例：区域原型存储结构
  geo_prototypes = {
      "tropical": [p1, p2, ..., pN],  # 热带区域原型
      "arctic": [q1, q2, ..., qM],    # 极地区域原型
      ...
  }
  ```
...
### 5.1 典型应用场景
- **灾害监测**：结合 SAR 时序数据进行洪水/地震变化预测
- **农业评估**：利用多光谱时序分析作物生长周期
- **城市规划**：通过多粒度对比实现土地利用分类
```
